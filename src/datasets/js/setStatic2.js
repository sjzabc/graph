...
function func(params) {
    ...
    var newNode,texts,nodeLink={},storeId=[];
    var force/*  = d3
    ...
    async function setXY(nl,data,nodeId,angle,radian){
        // console.log(nodeId)
        // console.log(nl)
        if(storeId.indexOf(nodeId)==-1) storeId.push(nodeId); else return ;
        let nd = 0,count = 0;
        nl.links.forEach((id,i)=>{
            if(storeId.indexOf(id)==-1){
                let len,ra;
                if(nl.links.length>12){
                    let ct = parseInt(nl.links.length/12)+1;//3
                ...
                let lr = nodeLink[id].links.length>10?360/nodeLink[id].links.length:35;
    ...
    async function format(data,l){
        let max=0,maxNode='',n=0;
        data.nodes.forEach((node,ind)=>{
            if(storeId.indexOf(node.id)==-1){
                nodeLink[node.id]?'':nodeLink[node.id] = {
                    ind: ind,
                    links: [],
                };
                let a = nodeLink[node.id];
                data.links.forEach((link)=>{
                    if((link.source==node.id&&a.links.indexOf(link.target)==-1) || (link.target==node.id)&&a.links.indexOf(link.source)==-1){
                        a.links.push(link.source==node.id?link.target:link.source);
                    }
                })
                if(a.links.length>=max){
                    max = a.links.length;
                    maxNode = node.id;
                }
            }else{
        ...
        Object.assign(nodeLink[maxNode],a);
        Object.assign(data.nodes[nodeLink[maxNode].ind],nodeLink[maxNode]);
        let angle = parseFloat(360)/max;
        setXY(nodeLink[maxNode],data,maxNode,angle,angle).then(()=>{
            // console.log(storeId);
            // console.log(data.nodes);
            if(storeId.length<data.nodes.length){
                format(data,storeId.length);
            } else{
